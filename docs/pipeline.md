...
## Пример `curl`
Ниже показан пример запроса на эндпоинт `/analyze` с тестовыми данными. При необходимости замените текст и язык (`ru` или `en`).

```bash
curl -X POST "http://localhost:8000/analyze" \
  -H "Content-Type: application/json" \
  -d '{
        "text": "Иван Иванов, паспорт 4012 345678 выдан 12.03.2015, телефон +7 (912) 000-00-00",
        "language": "ru"
      }'
```

Ответ включает список обнаруженных сущностей с типом, позицией, исходным текстом и score.
